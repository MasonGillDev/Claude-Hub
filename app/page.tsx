...
import { getProjects, type Project } from "@/lib/claude";
...
export default function Home() {
  ...
  return (
    <div className="animate-fade-up">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-ink-soft">
            {hasRemoteDevices
              ? "Grouped by device. Pick one to dive in."
              : `${projects.length} project${projects.length === 1 ? "" : "s"} with Claude Code sessions. Pick one to dive in.`}
          </p>
        </div>
        <NewProjectButton />
      </div>
      ...
      {hasRemoteDevices ? (
        <CollapsiblePanel
          storageKey="device-panel:local"
          accent="linear-gradient(135deg,#7dd3fc 0%,#a5b4fc 100%)"
          header={
            <>
...
function LocalProjectCard({ p }: { p: Project }) {
  return (
    <div
    ...
    >
      ...
      <div className="h-20 w-full" style={{ background: gradientFor(p.id) }} />
